...
def img_id_to_int(img_id):
    img_id_int = 0
    if img_id[0] == 'D':
      img_id_int = 1 * (10**6) + int(img_id[1:]) * (10**3) 
    elif img_id[0] == 'K':
      img_id_int = 2 * (10**6) + int(img_id[1:]) * (10**3)
    elif img_id[0] == 'M':
      img_id_int = 3 * (10**6) + int(img_id[1:]) * (10**3)
    elif img_id[0] == 'P':
      img_id_int = 4 * (10**6) + int(img_id[1:]) * (10**3)
    elif img_id[0] == 'S':
      img_id_int = 5 * (10**6) + int(img_id[1:]) * (10**3)
...
def conv_img_id(dict_list):
    for dictionary in dict_list:
      ...
      dictionary["image_id"] = img_id_to_int(img_id)
    return dict_list
...
for mode in mode_list:
  DatasetCatalog.register("ds3_" + mode, lambda mode=mode: json_to_ds(path1 + mode + ".json"))
  MetadataCatalog.get("ds3_" + mode).set(thing_classes=["cell"])

cell_meta = MetadataCatalog.get("ds3_train")

#hyperparams
cfg = get_cfg()
cfg.merge_from_file(model_zoo.get_config_file("COCO-Detection/fast_rcnn_R_50_FPN_1x.yaml")) #TBD, this arch for now
cfg.DATASETS.TRAIN = ("ds3_train",) # 
cfg.MODEL.ROI_HEADS.SCORE_THRESH_TEST = 0.7 #have this for now
cfg.DATASETS.TEST = ("ds3_val",) #

cfg.DATALOADER.NUM_WORKERS = 2 #if memory doesnt get too loaded can increase
cfg.MODEL.WEIGHTS = model_zoo.get_checkpoint_url("COCO-Detection/fast_rcnn_R_50_FPN_1x.yaml")  #TBD, these weights for now
...
for d in random.sample(ds_train_meta, 3):
...
evaluator = COCOEvaluator("ds2_val", ("bbox", "segm"), False, output_dir="./output/")
val_loader = build_detection_test_loader(cfg, "ds2_val")
